...
async function startDevEnvironment() {
  console.log('[dev-startup] Starting development environment with file watching...');
  ...
  console.log('[dev-startup] Development environment ready with automatic file watching');
  console.log('[dev-startup] Monitoring src/content/kommunen/ for changes');
  console.log('[dev-startup] Use Ctrl+C to stop both Astro server and file watcher');
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  startDevEnvironment().catch(error => {
    console.error('[dev-startup] Failed to start development environment:', error);
    process.exit(1);
  });
}
