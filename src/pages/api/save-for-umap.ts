...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!geoJsonString || geoJsonString.length < 20) {
       return new Response(JSON.stringify({ error: 'Ungültige GeoJSON-Daten empfangen.' }), {
        status: 400,
      });
    }

    // 2. Ziel-Pfad definieren
    const targetDir = path.join(process.cwd(), 'public', PUBLIC_DATA_PATH);
    ...
    console.log(`[API save-for-umap] 🗺️ uMap-Daten erfolgreich gespeichert in: ${filePath}`);
    ...
    console.error('[API save-for-umap] Fehler beim Speichern der uMap-Daten:', error);
    return new Response(JSON.stringify({ error: 'Speichern der uMap-Daten fehlgeschlagen.' }), {
      status: 500,
    });
  }
};
